...
public class PacketMoneyData {
   ...
   public static void handle(PacketMoneyData pck, Supplier<NetworkEvent.Context> ctxSupplier) {
      if (((NetworkEvent.Context)ctxSupplier.get()).getDirection().getReceptionSide() == LogicalSide.CLIENT) {
         ((NetworkEvent.Context)ctxSupplier.get()).enqueueWork(() -> handleClientUpdate(pck));
      }
   ...
   private static void handleClientUpdate(PacketMoneyData pck) {
      Minecraft.func_71410_x().field_71439_g.getCapability(CapabilityMoney.MONEY_CAPABILITY).ifPresent((capa) -> capa.setMoney(pck.money));
   }
}
