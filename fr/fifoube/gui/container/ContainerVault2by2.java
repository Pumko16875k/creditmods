...
public class ContainerVault2by2 extends Container {
   ...
   public ContainerVault2by2(int windowId, PlayerInventory playerInv, BlockPos pos) {
      ...
      if (entity instanceof TileEntityBlockVault2by2) {
         ...
         for(int i = 0; i < 6; ++i) {
            for(int j = 0; j < 9; ++j) {
               this.func_75146_a(new SlotItemHandler(inventory, j + i * 9, 8 + j * 18, i * 18 - 10));
            }
         }
      }
   ...
   private void bindPlayerInventory(PlayerInventory inventoryPlayer) {
      for(int i = 0; i < 3; ++i) {
         for(int j = 0; j < 9; ++j) {
            this.func_75146_a(new Slot(inventoryPlayer, j + i * 9 + 9, j * 18 + 8, i * 18 + 112));
         }
      }

      for(int var4 = 0; var4 < 9; ++var4) {
         this.func_75146_a(new Slot(inventoryPlayer, var4, 8 + var4 * 18, 170));
      }
